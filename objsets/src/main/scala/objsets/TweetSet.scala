...
import objsets.Main.greetingsLiviu

case class Tweeet(val user: String, val text: String, val retweets: Int)
  ...
  def descendingByRetweet: TweetList = {
    def loop(set: TweetSet, acc: TweetList): TweetList = {
      if (set.isEmpty) acc
        ...
        val most = set.leastRetweeted
        loop(set.remove(most), new Cons(most, acc))
      }
    }
...
class Empty extends TweetSet:
  ...
  def leastRetweeted: Tweet = throw new NoSuchElementException("Empty.leastRetweeted")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = {
    ...
    val leftMost = if (!left.isEmpty) maxRetweet(left.mostRetweeted, elem) else elem
    val rightMost = if (!right.isEmpty) maxRetweet(right.mostRetweeted, elem) else elem
    maxRetweet(leftMost, rightMost)
  }

  def leastRetweeted: Tweet = {

    def minRetweet(t1: Tweet, t2: Tweet): Tweet = if (t1.retweets < t2.retweets) t1 else t2

    val leftMost = if (!left.isEmpty) minRetweet(left.mostRetweeted, elem) else elem
    val rightMost = if (!right.isEmpty) minRetweet(right.mostRetweeted, elem) else elem
    minRetweet(leftMost, rightMost)
  }

  def union(that: TweetSet): TweetSet = {
    val leftRightUnion = left.union(right.union(that))
    leftRightUnion.incl(elem)
  }
