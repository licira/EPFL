...
object Anagrams extends AnagramsInterface:
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = {
    s.flatMap(word => wordOccurrences(word))
      .groupBy((c, f) => c)
      .map((c, list) => (c, list.map(_._2).sum))
      .toList
      .sortBy(_._1)
  }
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    ...
    dictionary.groupBy(wordOccurrences).view.mapValues(_.toList).toMap
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val emptyOccurrence: Occurrences = List()

    occurrences.foldRight(List(emptyOccurrence)) { case ((char, count), acc) =>
      for {
        combination <- acc
        n <- 0 to count
      } yield if (n == 0) combination else (char, n) :: combination
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    for {
      (char, count) <- x
      yCount = yMap.getOrElse(char, 0) // Get the count from yMap safely
      if yCount <= count
      newCount = count - yCount
      if newCount > 0
    } yield (char, newCount)
  }
    ...
    def anagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
        ...
        for {
          ...
          restAnagrams <- anagrams(subtract(occurrences, combination))
